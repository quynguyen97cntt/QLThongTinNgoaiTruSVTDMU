...
class TKTheoPhuongExport implements FromView, ShouldAutoSize, WithEvents
{
    public function view(): View
    {
        return view('pages.admin.xuattktheophuong', [
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                $thongke=DB::select('select phuong.gid, phuong.tenphuong, count(ST_Contains(phuong.geom, tro.geom)) as sl
                from vungranhgioiphuongtxtdm_region phuong join khunhatro_tdm_point tro
                on ST_Contains(phuong.geom, tro.geom)=True
                group by phuong.gid, phuong.tenphuong ORDER BY sl DESC');
                $soluong=0;
